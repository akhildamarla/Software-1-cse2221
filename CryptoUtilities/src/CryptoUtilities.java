...
public final class CryptoUtilities {
    ...
    private static final int WITNESSLIMIT = 50;
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        //wTemp holds value for w as operations happen on it
        NaturalNumber wTemp = new NaturalNumber2(0);
        wTemp.copyFrom(w);

        //checks first part of fermat's little theorem
        powerMod(w, new NaturalNumber2(2), n);
        if (w.compareTo(new NaturalNumber2(1)) == 0) {
            isWitness = true;
        }

        //checks second part of fermat's little theorem
        w.copyFrom(wTemp);
        ...
        //returns w to its initial value and returns isWitness
        w.copyFrom(wTemp);
        return isWitness;
